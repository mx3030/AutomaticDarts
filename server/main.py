...
def read_config(config_path):
    ...
    server_config = {
        ...
        'app_main': config.get('Server', 'app_main'),
        'pipes_dir': config.get('Server', 'pipes_dir'),
    }
...
if __name__ == '__main__':
    ...
    APP_MAIN = server_config['app_main']
    PIPES_DIR = server_config['pipes_dir']

    # Start WebSocket server in different thread
    ws_thread = threading.Thread(target=start_ws, args=(HOST, WS_PORT, SSL_KEY, SSL_CERT, APP_MAIN, PIPES_DIR))
    ws_thread.start()
